...
{% endcodeblock %}
 ...
 Но, что произойдет теперь, если параметр `:status` равен `nil` или `blank`.
...
{% endcodeblock %}
 ...
 Когда создаем скоуп можем добавить специальные расширения, которые будут доступны только в нашем объекте
 при вызове этого скоупа. В случае kaminari, это только добавление страницы к `Active Record` модели
